...
const AllOrders = () => {
    ...
    const fetchAllOrders =async()=>{
        ...
        // console.log("all orders",dataApi.data)
    }
    useEffect(()=>{
        fetchAllOrders()
    },[])
          ...
          data.map((item, index) => {
            return (
              <div key={index} className='p-2' >
                ...
                <p className='font-medium text-lg '>Order Id : {item?._id}</p>
                <div className='border rounded'>
                  <div className='grid gap-2 p-2'>
                    <p className='font-medium text-lg text-slate-500 '>User Email : {item.email}</p>
                    {
